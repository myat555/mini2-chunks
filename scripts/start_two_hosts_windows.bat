...
start "Node A (Leader)" cmd /c "cd /d .. && python -u node.py %CONFIG% A > logs\node_a.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process B (Team Leader) on 192.168.1.2:60052...
start "Node B (Team Leader)" cmd /c "cd /d .. && python -u node.py %CONFIG% B > logs\node_b.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process D (Worker) on 192.168.1.2:60054...
start "Node D (Worker)" cmd /c "cd /d .. && python -u node.py %CONFIG% D > logs\node_d.log 2>&1 && pause"
timeout /t 2 /nobreak >nul
