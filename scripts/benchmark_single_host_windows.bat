...
:: Start all servers with logging to logs folder
echo Starting Process A (Leader)...
start "Node A" cmd /c "python -u node.py %CONFIG% A > logs\node_a.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process B (Team Leader)...
start "Node B" cmd /c "python -u node.py %CONFIG% B > logs\node_b.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process C (Worker)...
start "Node C" cmd /c "python -u node.py %CONFIG% C > logs\node_c.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process D (Worker)...
start "Node D" cmd /c "python -u node.py %CONFIG% D > logs\node_d.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process E (Team Leader)...
start "Node E" cmd /c "python -u node.py %CONFIG% E > logs\node_e.log 2>&1 && pause"
timeout /t 2 /nobreak >nul

echo Starting Process F (Worker)...
start "Node F" cmd /c "python -u node.py %CONFIG% F > logs\node_f.log 2>&1 && pause"
timeout /t 2 /nobreak >nul
...
python benchmark.py 127.0.0.1 60051 %REQUESTS% %CONCURRENCY% logs
...
echo Process logs saved to logs\node_*.log
